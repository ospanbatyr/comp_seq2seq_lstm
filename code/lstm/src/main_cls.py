...
def main():
	...
	wandb.init(project=config.project_name, entity='taninzrt')
	...
	if is_train:
		...
		beam_decode = config.beam_decode
		outputs_path = config.outputs_path
		topk = config.topk
		with open(config_file, 'rb') as f:
			config = AttrDict(pickle.load(f))
			config.gpu = gpu
			config.beam_decode = beam_decode
			config.conf = conf
		...
		epoch_offset, min_train_loss, min_val_loss, min_test_loss, min_gen_loss, max_train_acc, max_val_acc, max_test_acc, max_gen_acc, best_epoch, voc1 = load_checkpoint(model, config.mode, pretrained_checkpoint, logger, device, pretrained = True)
		...
		if config.mode == 'test':
			gen_acc_epoch = run_validation(config=config, model=model, dataloader=gen_dataloader, disp_tok='GEN', voc1=voc1, device=device, logger=logger, epoch_num = 0)
