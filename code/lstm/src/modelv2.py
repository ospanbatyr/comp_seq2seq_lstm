...
class Seq2SeqModel(nn.Module):
	...
	def beam_decode(self, src, input_seq1=None, input_seq2=None, input_len1=None, input_len2=None, validation=False, return_probs=False, beam_width=20, topk=3):
		...
		with torch.no_grad():
			# Just copy-pasted this code snippet.
			if self.config.embedding == 'bert' or self.config.embedding == 'roberta':
				input_seq1, input_len1 = self.embedding1(src)
				input_seq1 = input_seq1.transpose(0,1)
				sorted_seqs, sorted_len, orig_idx = sort_by_len(input_seq1, input_len1, self.device)
			...
			if self.config.cell_type == 'lstm':
				decoder_hiddens = (encoder_hidden[0][:self.decoder.nlayers], encoder_hidden[1][:self.decoder.nlayers])
			else:
				decoder_hiddens = encoder_hidden[:self.decoder.nlayers]

			# decoding goes sentence by sentence
			for idx in range(ex_count):

				if self.config.cell_type == 'lstm':
					decoder_hidden = (decoder_hiddens[0][:,idx, :].unsqueeze(1), decoder_hiddens[1][:,idx, :].unsqueeze(1))
					# print(f'Decoder_hidden size: {decoder_hidden}\n\n')
				else:
				...
				while True:
					...
					if n.word_id.item() == self.EOS_token and n.prev_node is not None:
						endnodes.append((score, n))

						# if we reached max num of sentences required
					...
					if self.config.use_attn:
						decoder_output, decoder_hidden, decoder_attention, _ = self.decoder(decoder_input, decoder_hidden, encoder_output)
...
def run_validation(config, model, dataloader, disp_tok, voc1, voc2, device, logger, epoch_num, validation = False):
	...
	for data in dataloader:
		...
		if config.mode == 'test' and config.beam_decode:
			if config.debug:
				print(f'Sent1_var (input_seq1): {sent1_var}\n\n')
				print(f'Sent2_var (input_seq2): {sent2_var}\n\n')
				print(f'input_len1: {input_len1}\n\n')
				print(f'input_len2: {input_len2}\n\n')
				print(f'sent1_var.size(1): {sent1_var.size(1)}\n\n')
				print(f'len(decoder_output): {len(decoder_output)}\n\n')
				print(f'[len(a) for a in my_tuple_2d]:  {[len(a) for a in decoder_output]}\n\n')

			decoder_output = model.beam_decode(src, sent1_var, sent2_var, input_len1, input_len2, validation, topk=config.topk)

			

		temp_acc_cnt, temp_acc_tot, disp_corr = cal_score(decoder_output, data['trg'], beam_decode=config.beam_decode)
		...
		with open(config.outputs_path + '/outputs.txt', 'a') as f_out:
			for i in range(len(sent1s[:display_n])):
				try:
					...
					if config.beam_decode:
						for k, query in enumerate(decoder_output[i]):
							f_out.write(f'Generated {k+1}: {query} -- Result: {disp_corr[i][k]}\n')
					else:
