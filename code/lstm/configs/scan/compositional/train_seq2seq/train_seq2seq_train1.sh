...
# DO USE TOKEN BELOW
python -m src.main -mode train -project_name test_runs -hidden_size 512 -depth 3 -model_selector_set val -pretrained_model_name none -finetune_data_voc none -dev_set -no-test_set -no-gen_set -dataset scan/compositional/train_1 -dev_always -no-test_always -no-gen_always -epochs 250 -save_model -show_train_acc -embedding random -no-freeze_emb -no-freeze_emb2 -no-freeze_lstm_encoder -no-freeze_lstm_decoder -no-freeze_fc -batch_size 256 -lr 0.001 -emb_lr 0.0005 -dropout 0.1 -no_beam_decode -early_stopping 75 -run_name scan_comp_train1 -gpu 0 -topk 1
# todo change epoch
